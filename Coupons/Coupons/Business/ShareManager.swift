...
protocol ShareManagerDelegate: NSObjectProtocol {
 ...
 class ShareManager: NSObject,FBRequstHandlerDelegate{
   ...
   func fbShare(_ coup: Offer?, atControl share: CouponDetailViewController?) {
        self.selectedCoup = coup
        self.sharecont = share
    
        if  Utility.instance.handleNetwork(){
            shareMethod(coup, atControl: share)
        }
    }

    
// MARK: - handle fbsharing
       private func shareMethod(_ coup: Offer?, atControl control: CouponDetailViewController?) {
        FBRequstHandler.sharedInstance.delegate = self
        FBRequstHandler.sharedInstance.shareToFB(coup: coup, atControl: control)
}
